 ...
 */
@CordaSerializable
sealed class ByteSequence(protected val _bytes: ByteArray, val offset: Int, val size: Int) : Comparable<ByteSequence> {
    /**
    ...
    /** Returns a [subSequence] of all but the first n bytes of this sequence. */
...
open class OpaqueBytes(bytes: ByteArray) : ByteSequence(bytes, 0, bytes.size) {
     ...
     * this later.
     */
    override final val bytes get() = _bytes.clone()

    fun writeTo(output: OutputStream) = output.write(_bytes)
    fun putTo(buffer: ByteBuffer) = buffer.put(_bytes)!!
}
 ...
 */
class OpaqueBytesSubSequence(bytes: ByteArray, offset: Int, size: Int) : ByteSequence(bytes, offset, size) {
    override val bytes get() = _bytes
